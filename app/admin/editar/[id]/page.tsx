...
function EditarPerfume({ id }: { id: string }) {
    ...
    async function fetchProducto() {
      ...
      if (error) {
        ...
        setForm({
          nombre: data.nombre || '',
          marca: data.marca || '',
          categoria: data.categoria || CATEGORIAS_PREDETERMINADAS[0],
          descripcion: data.descripcion || '',
          precio: data.precio?.toString() || '',
          costo: data.costo?.toString() || '',
          stock: data.stock?.toString() || '',
          stock_minimo: data.stock_minimo?.toString() || '3',
        })
        ...
        const fotosIniciales = galeriaBd.map((url, i) => ({
          id_local: `bd-${i}-${Date.now()}`,
          esNueva: false,
          urlPreview: url
        }))
          ...
          try {
             if (data.tamano.startsWith('[')) setVariantesVisibles(JSON.parse(data.tamano))
                ...
                const parsed = data.tamano.split(',').map((item: string) => {
                  ...
                  return { nombre: n?.trim(), precio: Number(p?.trim()) || 0, agotado: false }
                }).filter((v: any) => v.nombre)
                setVariantesVisibles(parsed)
             }
          } catch(e) {}
        }
        ...
        if (data.etiquetas) {
          try {
            if (data.etiquetas.startsWith('[')) {
               const parsed = JSON.parse(data.etiquetas)
               setInsignias(parsed.filter((t: any) => t.tipo === 'insignia').map((i: any) => ({texto: i.texto, color: i.color})))
               setTags(parsed.filter((t: any) => t.tipo === 'tag').map((t: any) => t.texto))
            } else {
               const array = data.etiquetas.split(',').map((t: string) => t.trim())
               const insig: any[] = []
               const desc: string[] = []
               array.forEach((t: string) => {
                   const txt = t.toLowerCase()
                   if (txt === 'top' || txt === 'top ventas') insig.push({texto: 'TOP', color: 'bg-black text-white'})
                   else if (t.includes('%')) insig.push({texto: t, color: 'bg-[#e50000] text-white'})
                   else desc.push(t)
               })
               setInsignias(insig)
               setTags(desc)
  ...
  const handleAgregarImagenes = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      const nuevas: FotoUnificada[] = Array.from(e.target.files).map((file, i) => ({ 
        id_local: `new-${i}-${Date.now()}`,
        esNueva: true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const foto of fotos) {
        if (!foto.esNueva) {
          urlsFinales.push(foto.urlPreview)
        } else if (foto.file) {
  ...
  if (cargando) return <div className="min-h-screen flex items-center justify-center bg-[#F4F5F7] uppercase text-sm text-gray-500">Cargando...</div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {fotos.map((foto, index) => (
                    <div 
                      ...
                      className={`relative bg-gray-50 rounded-lg border-2 overflow-hidden group h-32 cursor-grab active:cursor-grabbing transition-colors ${
                        index === 0 ? 'border-black' : (foto.esNueva ? 'border-amber-400 border-dashed bg-amber-50/20' : 'border-gray-200')
                      }`}
                    >
                      ...
                      <button 
                        ...
                        onClick={(e) => { 
                          e.stopPropagation(); 
                          if(!foto.esNueva) { if(!window.confirm('¿Borrar foto de la base de datos?')) return; }
