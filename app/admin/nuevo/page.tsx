...
export default function NuevoPerfume() {
  ...
  const [form, setForm] = useState({
    ...
    precio: '', costo: '', stock: '', stock_minimo: '3',
  })
  ...
  const handleDragOver = (e: React.DragEvent, index: number) => {
    ...
    const items = [...imagenes]; const draggedItem = items[draggedIndex];
    items.splice(draggedIndex, 1); items.splice(index, 0, draggedItem);
    setImagenes(items); setDraggedIndex(index);
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CONSTRUIMOS EL JSON PERFECTO Y LIMPIO
      ...
      if (descuento > 0) etiquetasCombinadas.push({ tipo: 'descuento', valor: descuento, texto: `-${descuento} OFF`, color: 'bg-[#e50000] text-white' })
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="bg-white border-b border-gray-200 px-8 py-5 flex justify-between items-center sticky top-0 z-20 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleSubmit} disabled={guardando} className="bg-black text-white px-8 py-2.5 rounded-md text-sm font-bold tracking-wider uppercase hover:bg-gray-800 disabled:opacity-50 transition-colors">{guardando ? 'Guardando...' : 'Crear Producto'}</button>
          </div>
        </header>
        ...
        <div className="p-8 overflow-y-auto flex-1">
          <form className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* LA NUEVA SECCIÓN DE MARKETING 100% INDEPENDIENTE */}
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  ...
                  {/* DESCUENTO DIRECTO */}
                  <div className={`flex items-center gap-3 p-4 rounded-xl border transition-colors ${descuento > 0 ? 'bg-rose-50 border-rose-200' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="flex-1">
                      <span className="block font-bold text-gray-900 mb-1">% Descuento</span>
                      <input type="number" min="0" max="99" value={descuento || ''} onChange={e => setDescuento(Number(e.target.value))} placeholder="Ej. 20" className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-black" />
                    </div>
                    {descuento > 0 && <span className="bg-[#e50000] text-white text-[10px] font-black px-2 py-1 rounded uppercase mt-5">-{descuento}% OFF</span>}
                  </div>
                </div>
